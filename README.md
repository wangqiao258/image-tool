...
带图形界面的批量图片处理工具，适合不懂技术的同事使用。

## 适用场景

设计师、运营同学每次上新或发平台前，常常要处理几百张商品图：统一尺寸 → 按规则重命名 → 加出血线，手工逐张处理要几小时，既耗时又容易漏。

本工具把这三步合并到一个界面里，勾选功能、点一下就能批量跑完——**不懂技术的业务同事也能独立使用**，不用每次找开发。

## 界面预览

![界面预览](docs/gui-screenshot.png)
